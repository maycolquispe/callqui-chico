...
// Filtrar por lote si se pasa como parámetro
$lote_id_filtro = isset($_GET['lote_id']) ? intval($_GET['lote_id']) : 0;
$where = "1=1";
if ($lote_id_filtro > 0) {
    $where .= " AND t.lote_id = $lote_id_filtro";
    
    // Obtener info del lote
    $stmtLote = $conn->prepare("SELECT lote, manzana, sector FROM lotes WHERE id = ?");
    $stmtLote->bind_param("i", $lote_id_filtro);
    $stmtLote->execute();
    $loteInfo = $stmtLote->get_result()->fetch_assoc();
    $stmtLote->close();
}
    ...
    <div class="main-container">
        ...
        <div class="page-header">
            <div>
                ...
                <?php if ($lote_id_filtro > 0 && $loteInfo): ?>
                <div class="mt-2" style="background: rgba(37,99,235,0.2); padding: 0.5rem 1rem; border-radius: 8px; display: inline-block;">
                    <small><i class="bi bi-geo-alt me-1"></i>Filtrado por: <strong>Lote <?= $loteInfo['lote'] ?> - Manzana <?= $loteInfo['manzana'] ?> (<?= $loteInfo['sector'] ?>)</strong></small>
                    <a href="historial.php" class="ms-2 text-white"><i class="bi bi-x-lg"></i></a>
                </div>
                <?php endif; ?>
            </div>
        </div>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php if ($t['nombre_anterior']): ?>
                                    ...
                                    <?php if (!empty($t['copropietario_anterior'])): ?>
                                        <br><small class="text-muted">+ <?= htmlspecialchars($t['copropietario_anterior']) ?></small>
                                    <?php endif; ?>
                                <?php else: ?>
                            ...
                            <td>
                                <strong><?= htmlspecialchars($t['nombre_nuevo'] . ' ' . $t['apellido_nuevo']) ?></strong>
                                <?php if (!empty($t['copropietario_nuevo'])): ?>
                                    <br><small class="text-info">+ <?= htmlspecialchars($t['copropietario_nuevo']) ?></small>
                                <?php endif; ?>
                            </td>
                            <td><small><?= htmlspecialchars($t['nombre_registro']) ?></small></td>
                            <td>
                                <?php if ($t['documento']): ?>
                                    <a href="uploads/<?= htmlspecialchars($t['documento']) ?>" target="_blank" class="btn btn-sm btn-outline-light">
                                        <i class="bi bi-file-earmark-pdf"></i> Ver
                                    </a>
                                    <a href="generar_certificado_transferencia.php?id=<?= $t['id'] ?>" target="_blank" class="btn btn-sm" style="background: rgba(245,158,11,0.2); color: #f59e0b; border: 1px solid rgba(245,158,11,0.3);">
